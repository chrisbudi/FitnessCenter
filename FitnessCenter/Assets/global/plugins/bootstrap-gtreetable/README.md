...
GTreeTable is extension of [Tweeter Bootstrap 3](https://getbootstrap.com) framework, which allows to use tree structure inside HTML table.
...
Test available on [demo project](https://gtreetable2.gilek.net).

![](https://gilek.net/images/gtt2-demo.png)
...
Moment of saving and deleting node can goes together witch server communication by AJAX. Responsible actions: `onSave` and `onDelete`. It's should be functions that return [jQuery.ajax](https://api.jquery.com/jquery.ajax/) settings.
...
![](https://gilek.net/images/gtt2-pointer.png)

In this case, application is using additional libraries such as: [jQueryUI](https://jqueryui.com/) and [jQuery Browser Plugin](https://github.com/gabceb/jquery-browser-plugin) so it's necessary to include required files in the code:
...
![](https://gilek.net/images/gtt2-type.png) 
...
+ `draggable` (boolean) - define whether nodes can be moved. Parameter value changing on true is related with necessity of adding required [jQueryUI](https://jqueryui.com/) library:
  + core,
  + widget,
  + position,
  + mouse,
