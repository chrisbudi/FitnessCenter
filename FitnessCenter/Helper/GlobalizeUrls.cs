...
namespace FitnessCenter.Helper
{
    public static class GlobalizeUrls
    {
        ...
        {
            get { return "~/Scripts/globalize.js"; }
        }
        ...
        public static string GlobalizeCulture
        {
            get
            {
                ...
                var regionalisedFileToUse = string.Format(filePattern, "id-ID"); //Default localisation to use
                ...
                if (File.Exists(HostingEnvironment.MapPath(string.Format(filePattern, currentCulture.Name))))
                    //First try for a globalize.culture.en-GB.js style file
                    regionalisedFileToUse = string.Format(filePattern, currentCulture.Name);
                else if (
                    File.Exists(
                        HostingEnvironment.MapPath(string.Format(filePattern,
                            currentCulture.TwoLetterISOLanguageName))))
                    //That failed; now try for a globalize.culture.en.js style file
