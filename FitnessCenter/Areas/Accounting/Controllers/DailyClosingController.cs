    ...
    public class DailyClosingController : Controller
    {
        ...
        public ActionResult ClosingMembership(int[] membershipId, FormCollection form)
        {
            ...
            using (var ts = new TransactionScope())
            {
                ...
                {
                    member.AccountingStatus = EnumAcountingStatus.Closed.ToString("F");
        ...
        public ActionResult ClosingPersonalTrainer(int[] ptId, FormCollection form)
        {
            ...
            using (var ts = new TransactionScope())
            {
                foreach (var member in members)
                {
                    member.trMembership.AccountingStatus = EnumAcountingStatus.Closed.ToString("F");
        ...
        {
            decimal? amount = 0;
            var membership = new DataServiceMembership().LoadDataClosing(requestModel, EnumDaily.Membership, EnumAcountingStatus.Null, out amount);
        ...
        public ActionResult TrainerResult([ModelBinder(typeof(DataTablesBinder))] IDataTablesRequest requestModel)
        {
            ...
            decimal? amount = 0;
            var membership = new DataServicePersonalTrainer().LoadDataClosing(requestModel, EnumDaily.PersonalTrainer, EnumAcountingStatus.Null, out amount);
            var count = requestModel.Start + 1;
            var result = from d in membership.ListClass
                select new[]
                {
